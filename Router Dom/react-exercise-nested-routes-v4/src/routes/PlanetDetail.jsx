...
const PlanetDetail = () => {
  ...
  useEffect(() => {
    const loadDetail = async () => {
      setLoading(true);
      try {
        const url = `https://swapi.py4e.com/api/planets/${id}`; // Fetch planet details by ID
        const data = await (await fetch(url)).json(); // Directly retrieve and parse JSON data
        setDetail(data); // Set planet detail from the fetched data
      } catch (error) {
        console.log(error);
      }
      setLoading(false);
